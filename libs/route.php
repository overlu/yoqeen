...
class Route
{
    ...
    protected function callback($callback, array $args = [])
    {
        if (isset($callback))
        {
            ...
            {
                $fixcallback       = explode('@', $callback);
                $countcall = count($fixcallback);
                if($countcall>2)
                {
                    $yoqeenMod = $fixcallback[0];
                    $yoqeenCon = $fixcallback[1];
                    $yoqeenFun = $fixcallback[2];
                    
                } elseif ($countcall==2) {
                    $yoqeenMod = $fixcallback[0];
                    $yoqeenCon = $fixcallback[0];
                ...
                $countcall = count($callback);
                if($countcall>2)
                {
                    $yoqeenMod = $callback[0];
                    $yoqeenCon = $callback[1];
                    $yoqeenFun = $callback[2];
                    
                } elseif ($countcall==2) {
                    $yoqeenMod = $callback[0];
                    $yoqeenCon = $callback[0];
                    $yoqeenFun = $callback[1];
                } else {
                    $yoqeenMod = $callback[0];
                    $yoqeenCon = $callback[0];
